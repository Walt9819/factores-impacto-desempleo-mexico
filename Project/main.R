...
## ADVERTENCIA: Este carga se debe ejecutar una sola vez, debido a que la informaci髇 
## ya se encuentra disponible en la base de datos MongoDB para fines de este proyecto a 
# fecha corte Diciembre 2020.En caso de hacer pruebas, cambiar url path de conexi髇.
## Ninguna de las variables definidas en esta secci髇 se deben considerar para an醠isis 
...
# Visualizaci贸n del empleo en M茅xico y su evoluci贸n mensual
# Se resalta la mayor ca铆da de empleos registrada en M茅xico, ocasionada principalmente por la pandemia COVID-19. 
# Donde la tasa de ocupaci贸n entre Febrero y Julio del 2020 cay贸 % perdiendo mas de X millones de puestos formales como informales.

plot_ly(data = data_chart1, x = ~date_month, y = ~asegurados, mode = 'lines', line = list(color = 'rgb(205, 12, 24)', width = 4)) %>% layout(title = "Empleo en M茅xico 2019 - 2020", 
